...
const UserJoin = () => {
    ...
    const JoinFunc = async (e) => {
        ...
        // 이름과 프로필 추가함(기본 이미지 링크를 넣어줌(네이버 클라우드에 미리 기본 이미지가 있어야함))
        await createdUser.user.updateProfile({
            displayName: youName,
            photoURL: "https://kr.object.ncloudstorage.com/react-blog05/user/Frame%206.jpg"
        });
        ...
        let body = {
            ...
            photoURL: "https://kr.object.ncloudstorage.com/react-blog05/user/Frame%206.jpg",    // 기본이미지 추가함
        }
        ...
        axios.post("/api/user/join", body)
            .then((response) => {
                if (response.data.success) {
                    alert("회원가입이 완료되었습니다.");
                    firebase.auth().signOut();  // 회원가입을 하면 바로 로그인하는 것을 방지해주는 것
                    navigate("/login");
                } else {
                    alert("회원가입이 실패하였습니다.");
                }
    ...
    const NameCheckFunc = (e) => {
        setFlag(false); // 중복검사를 회원가입 누르고 난 후에 누르고 다시 회원가입을 누를 때 작동할 수 있게 하는 것
